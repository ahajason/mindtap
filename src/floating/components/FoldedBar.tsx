...
export function FoldedBar({ taskTitle, focusMs, status }: FoldedBarProps) {
  const isEmpty = status === "empty";
  const title = isEmpty ? "未命名任务" : taskTitle;

  return (
    <div
      className="glass-l2 flex items-center justify-between rounded-full px-3 py-1 text-[12px]"
      role="status"
      aria-label={
        isEmpty
          ? "Mindtap 计时器，未开始任务"
