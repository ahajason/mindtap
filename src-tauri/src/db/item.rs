// 深模块 ItemRepo —— 藏住台账状态机 + 结算 + 查询(2026-08-02 拆分)。
// 外部 seam: create / start / pause / complete / rename / confirm_pending / triage / soft_delete / list。
// 失真检测(冷却/跨天)已独立到 db/dormant.rs(单向依赖 dormant → item)。
// 状态转换正确性(多并行 / focus_ms 只增不减 / 待确认结算)全部在此模块事务内原子完成。
...
pub(crate) fn get_by_id(conn: &Connection, id: i64) -> Result<Option<Item>, AppError> {
