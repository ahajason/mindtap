...
#[tauri::command]
pub fn timer_session_get_active(state: State<DbState>) -> Result<Option<timer_session::TimerSession>, AppError> {
    let conn = state.0.lock().map_err(|e| AppError::InvalidState(e.to_string()))?;
