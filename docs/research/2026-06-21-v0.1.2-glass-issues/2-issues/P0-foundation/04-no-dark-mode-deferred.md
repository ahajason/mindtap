# P0 · 无 Dark mode / macOS 系统外观适配(延后到 V0.1.3+)

> **优先级**: 🔴 P0 根因级 — 但**延后到 V0.1.3+ 处理**(用户决策 2026-06-21)
> **创建**: 2026-06-21
> **状态**: 🟡 **已记录,延后处理**
...
| 方向 | 改动量 | 风险 | 适合阶段 |
|---|---|---|---|
| **A. CSS 变量 + `@media (prefers-color-scheme: dark)`** | 中(全 token 翻倍 + 媒体查询) | 低 | V0.1.3 |
| **B. CSS 变量 + `[data-theme="dark"]` + React state** | 大(全局 theme provider + 所有页面读 theme) | 中 | V0.1.4 |
| **C. 跟随系统 + 用户可覆盖** | 最大(state 同步系统 + override UI) | 高 | V0.2.x |

## 四、为什么延后到 V0.1.3+

1. **架构性改动**:不是 1-2 行 CSS,而是 **token 全量翻倍 + Theme provider + 系统/用户双源**,工作量 **3-5 天**
2. **依赖 V0.1.2 的视觉稳定**:Dark mode 实施时,所有玻璃 token / shadow / border 都要重新调,跟 V0.1.2 的"穿透修复"+"focus 3 层"+"几何对齐"叠加,风险叠加
3. **测试矩阵翻倍**:每个视觉改动都要在 light + dark 下各验证一次
4. **用户决策明确**(2026-06-21):"L4/L5 太大,应该延后到 V0.1.3+,明确记录问题"
...
1. **V0.1.2 视觉全部稳定**(本目录 2-issues/ 全部关闭)
2. **有 ≥ 1 个用户明确反馈需要 Dark mode**
3. **V0.2.x 阶段规划开始**(届时 Theme 系统是基础)

## 八、自检清单(留作 V0.1.3+ 实施时)
